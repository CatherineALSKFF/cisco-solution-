...
export function Sidebar({ activeView, onViewChange, stats }: SidebarProps) {
  const sections = [
    {
      ...
      items: [
        ...
        { id: "risks", label: "Risk Center", icon: "alert", badge: stats?.high },
        { id: "renewals", label: "Renewals", icon: "refresh", badge: stats?.renewals },
    ...
    },
    {
      label: "LIBRARY",
      items: [
        { id: "contracts", label: "Contracts", icon: "file", badge: stats?.total },
        { id: "vendors", label: "Vendors", icon: "building" },
        { id: "clauses", label: "Clauses", icon: "bookmark" },
  ...
  const getIcon = (name: string) => {
    const icons: Record<string, React.ReactNode> = {
      ...
      bookmark: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" /></svg>,
  ...
  return (
    <motion.aside
      ...
      className="w-56 h-screen bg-[#1a1a1a] flex flex-col fixed left-0 top-0 z-30"
    >
      {/* Logo */}
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2 text-[13px] transition-all ${
                    activeView === item.id
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={activeView === item.id ? "text-white" : "text-white/40"}>
                      {getIcon(item.icon)}
                    </span>
